defmodule JobHuntingExWeb.UserAuthTest do
  use JobHuntingExWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias JobHuntingEx.Accounts
  alias JobHuntingEx.Accounts.Scope
  alias JobHuntingExWeb.UserAuth

  import JobHuntingEx.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, JobHuntingExWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      JobHuntingExWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "on_mount :require_authenticated" do
    ...
    test "redirects to login page if there isn't a valid user_token", %{conn: conn} do
      ...
      socket = %LiveView.Socket{
        endpoint: JobHuntingExWeb.Endpoint,
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        JobHuntingExWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
